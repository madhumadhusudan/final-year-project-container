  ...
  return (
    <article className="min-w-0 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
      ...
      {!selectedFile ? <ImageUploader onFileSelect={onFileSelect} error={uploadError} /> : <div>
        <ImagePreview previewUrl={previewUrl} filename={selectedFile.name} image={result?.image} detections={detections} faces={faces} plates={plates} cards={cards} documents={documents} sensitiveTexts={sensitiveTexts} showObjects={showObjects} showFaces={showFaces} showPlates={showPlates} showCards={showCards} showDocuments={showDocuments} showSensitiveText={showSensitiveText} isAnalyzing={analysisStatus === 'analyzing'} onLoaded={onImageLoaded} onError={onImageError} />
        {(detections.length > 0 || faces.length > 0 || plates.length > 0 || cards.length > 0 || documents.length > 0 || sensitiveTexts.length > 0) && <div className="mt-4 flex flex-wrap gap-x-6 gap-y-1">
