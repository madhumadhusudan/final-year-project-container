...
assert.equal(await evaluate(`document.body.innerText.includes('5 privacy regions protected') && document.body.innerText.includes('Main subject preserved')`), true)
...
await evaluate(`Array.from(document.querySelectorAll('button')).find(button=>button.innerText.trim()==='Protect Image').click()`)
await waitFor(`document.body.innerText.includes('4 privacy regions protected')`, 'toggle-specific result')
...
socket.close()
console.log('Day 10 browser flow passed: document overlay/control, risk, protection, download, and 375/768/1024/1440 responsive layouts.')
