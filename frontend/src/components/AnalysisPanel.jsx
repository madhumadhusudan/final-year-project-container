  ...
  return (
    <article className="min-w-0 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
      ...
      {!selectedFile ? <ImageUploader onFileSelect={onFileSelect} error={uploadError} /> : <div>
        <ImagePreview previewUrl={previewUrl} filename={selectedFile.name} image={result?.image} detections={detections} faces={faces} plates={plates} cards={cards} showObjects={showObjects} showFaces={showFaces} showPlates={showPlates} showCards={showCards} isAnalyzing={analysisStatus === 'analyzing'} onLoaded={onImageLoaded} onError={onImageError} />
        {(detections.length > 0 || faces.length > 0 || plates.length > 0 || cards.length > 0) && <div className="mt-4 flex flex-wrap gap-x-6 gap-y-1">
          ...
          <label className="inline-flex min-h-11 items-center gap-3 text-sm font-semibold text-slate-700"><input type="checkbox" checked={showPlates} onChange={(event) => setShowPlates(event.target.checked)} className="h-5 w-5 accent-amber-500" /> Show License Plates</label>
          <label className="inline-flex min-h-11 items-center gap-3 text-sm font-semibold text-slate-700"><input type="checkbox" checked={showCards} onChange={(event) => setShowCards(event.target.checked)} className="h-5 w-5 accent-emerald-600" /> Show Cards</label>
        </div>}
