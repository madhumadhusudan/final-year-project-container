...
assert.equal(await evaluate(`document.body.innerText.includes('Privacy Risk Score') && document.body.innerText.includes('/ 100') && document.body.innerText.includes('HIGH RISK')`), true, 'real risk dashboard must be visible')
...
assert.equal(await evaluate(`document.body.innerText.includes('4 privacy regions protected') && document.body.innerText.includes('Main subject preserved')`), true)
assert.equal(await evaluate(`document.body.innerText.includes('BEFORE PROTECTION') && document.body.innerText.includes('AFTER PROTECTION') && document.body.innerText.includes('63 points') && document.body.innerText.includes('90% reduction')`), true, 'risk reduction must be displayed')
...
await writeFile('../docs/day8-desktop-protection.png', Buffer.from(desktopScreenshot.data, 'base64'))
...
await evaluate(`Array.from(document.querySelectorAll('button')).find(button=>button.innerText.trim()==='Protect Image').click()`)
await waitFor(`document.body.innerText.includes('3 privacy regions protected')`, 'toggle-specific result')
...
await writeFile('../docs/day8-mobile-protection.png', Buffer.from(mobileScreenshot.data, 'base64'))
...
socket.close()
console.log('Day 8 browser flow passed: defaults, analysis gating, protect flow, method/strength, toggle, comparison, download, and 375/768/1024/1440 responsive layouts.')
